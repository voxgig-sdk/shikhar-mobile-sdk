...
// ShikharMobile SDK pipeline test
//
...
{
    private static function utility(): ShikharMobileUtility
    {
        return new ShikharMobileUtility();
    }
    ...
    private static function ctx(array $over = []): ShikharMobileContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ShikharMobileContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ShikharMobileOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ShikharMobileError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ShikharMobileOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ShikharMobileMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ShikharMobileOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ShikharMobileMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ShikharMobileOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ShikharMobileMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShikharMobileSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ShikharMobileMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ShikharMobileResponse([]),
            'result' => new ShikharMobileResult([]),
        ]);
        [, $err] = ShikharMobileMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ShikharMobileSpec([]),
            'response' => null,
            'result' => new ShikharMobileResult([]),
        ]);
        [, $err] = ShikharMobileMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ShikharMobileSpec(['step' => 's']),
            'response' => new ShikharMobileResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ShikharMobileResult([]),
        ]);
        [, $err] = ShikharMobileMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ShikharMobileSpec(['step' => 's']),
            'response' => new ShikharMobileResponse(self::resp(200, ['v' => 1])),
            'result' => new ShikharMobileResult([]),
        ]);
        [, $err] = ShikharMobileMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ShikharMobileSpec(['step' => 's']),
            'response' => new ShikharMobileResponse(self::resp(200, ['v' => 2])),
            'result' => new ShikharMobileResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ShikharMobileMakeResponse::call($ctx);
    ...
    {
        $preset = new ShikharMobileResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ShikharMobileSpec([]),
            'response' => new ShikharMobileResponse([]),
            'result' => new ShikharMobileResult([]),
        ...
        [$got, $err] = ShikharMobileMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ShikharMobileResult([])]);
        [, $err] = ShikharMobileMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ShikharMobileSpec([]), 'result' => null]);
        [, $err] = ShikharMobileMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShikharMobileSpec(['step' => 's']),
            'result' => new ShikharMobileResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ShikharMobileOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShikharMobileMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShikharMobileSpec(['step' => 's']),
            'result' => new ShikharMobileResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ShikharMobileOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShikharMobileMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShikharMobileResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ShikharMobileSpec([]),
            'result' => new ShikharMobileResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ShikharMobileMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShikharMobileMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ShikharMobileError('boom', 'boom');
        $utility->fetcher = function (ShikharMobileContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShikharMobileSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ShikharMobileMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ShikharMobileContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ShikharMobileMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ShikharMobileResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        ShikharMobileMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ShikharMobileContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShikharMobileSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ShikharMobileMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ShikharMobileResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ShikharMobileSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ShikharMobileMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ShikharMobileMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ShikharMobileMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ShikharMobileResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ShikharMobileDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ShikharMobileResult(['ok' => false])]);
        $this->expectException(ShikharMobileError::class);
        ShikharMobileDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ShikharMobileResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ShikharMobileMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ShikharMobileMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ShikharMobileBaseFeature();
        $b = new ShikharMobileBaseFeature();
        ShikharMobileFeatureAdd::call($ctx, $a);
        ShikharMobileFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ShikharMobileBaseFeature
    {
        $f = new ShikharMobileBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ShikharMobileFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ShikharMobileContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ShikharMobileSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShikharMobilePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        ShikharMobilePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ShikharMobileResponse(['headers' => null]),
            'result' => new ShikharMobileResult([]),
        ]);
        ShikharMobileResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        ShikharMobileResultBody::call($ctx);
