...
class ShikharMobileMakeError
{
    public static function call(?ShikharMobileContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ShikharMobileContext([], null);
        }
        $op = $ctx->op ?? new ShikharMobileOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ShikharMobileResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ShikharMobileError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ShikharMobileError('', $msg, $ctx);
        ...
        if ($err instanceof ShikharMobileError) {
