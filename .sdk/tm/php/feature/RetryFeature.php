...
// ShikharMobile SDK retry feature
...
class ShikharMobileRetryFeature extends ShikharMobileBaseFeature
{
    ...
    public function init(ShikharMobileContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ShikharMobileContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ShikharMobileContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ShikharMobileContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
