# ShikharMobile SDK primary utility test
...
from projectname_sdk import ShikharMobileSDK
from projectname_sdk.core.spec import ShikharMobileSpec
from projectname_sdk.core.result import ShikharMobileResult
from projectname_sdk.core.response import ShikharMobileResponse
from projectname_sdk.core.operation import ShikharMobileOperation
from projectname_sdk.core.error import ShikharMobileError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import ShikharMobileBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return ShikharMobileError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_done_basic(self):
        ...
        def subject(entry):
            ...
            except ShikharMobileError as e:
                return None, e
    ...
    def test_make_error_no_throw(self):
        ...
        ctx.result = ShikharMobileResult({
            "ok": False,
            "resdata": {"id": "safe01"},
        })
    ...
    def test_feature_init_basic(self):
        init_client = ShikharMobileSDK.test(None, None)
        ...
        class TestInitFeature(ShikharMobileBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(ShikharMobileBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_fetcher_blocked_test_mode(self):
        ...
        blocked_client = ShikharMobileSDK({
            "base": "http://localhost:8080",
            "system": {
                "fetch": mock_fetch,
            },
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = ShikharMobileSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = ShikharMobileSDK.test(None, setup_opts)
    ...
    def test_operator_basic(self):
        ...
        def subject(entry):
            ...
            if not isinstance(in_val, dict):
                in_val = {}
            op = ShikharMobileOperation(in_val)
            return {
                "entity": op.entity,
                "name": op.name,
                "input": op.input,
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = ShikharMobileSDK.test(None, setup_opts)
