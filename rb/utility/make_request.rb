...
module ShikharMobileUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ShikharMobileResponse.new({})
    ...
    if fetch_err
      ...
      response = ShikharMobileResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ShikharMobileResponse.new(fetched)
    else
