...
module ShikharMobileUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = ShikharMobileContext.new({}, nil)
    end
    op = ctx.op || ShikharMobileOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || ShikharMobileResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(ShikharMobileError) ? err.msg : err.to_s
    ...
    sdk_err = ShikharMobileError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(ShikharMobileError)
