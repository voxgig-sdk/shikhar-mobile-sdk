...
require_relative 'ShikharMobile_types'


class ShikharMobileSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ShikharMobileHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ShikharMobileHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ShikharMobileFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ShikharMobileUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ShikharMobileHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ShikharMobileHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ShikharMobileHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => ShikharMobileError.new(
        "#{op}_allow",
        "ShikharMobileSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ShikharMobileHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ShikharMobileError.new(
        "graphql_error", "ShikharMobileSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = ShikharMobileSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
