# ShikharMobile SDK Authentication entity
...
class AuthenticationEntity
  ...
  def data_set(args)
    if args
      @_data = ShikharMobileHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ShikharMobileHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Authentication, Hash] the created Authentication; raises ShikharMobileError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = ShikharMobileHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
