...
class AuthenticationEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHIKHAR_MOBILE_TEST_AUTHENTICATION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def authentication_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHIKHAR_MOBILE_TEST_AUTHENTICATION_ENTID"]
  ...
  if env["SHIKHAR_MOBILE_TEST_LIVE"] == "TRUE"
    ...
    client = ShikharMobileSDK.new(Helpers.to_map(merged_opts))
  end
