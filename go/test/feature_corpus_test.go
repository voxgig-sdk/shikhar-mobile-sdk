...
//
// NewShikharMobileSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ShikharMobileSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewShikharMobileSDK(opts)
}
...
func fcCandidates(client *sdk.ShikharMobileSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ShikharMobileSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ShikharMobileSDK, name string) any {
