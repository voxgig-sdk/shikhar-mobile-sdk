...
MCP server exposing the ShikharMobile SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `shikhar-mobile_list` | `entity`, optional `query` map | First page of records as JSON |
| `shikhar-mobile_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `shikhar-mobile_list` and
`shikhar-mobile_load` tools become available in new sessions.
