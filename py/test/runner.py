# ShikharMobile SDK test runner

from __future__ import annotations
import os
import json

from shikharmobile_sdk.utility.voxgig_struct import voxgig_struct as vs


class ShikharMobileTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ShikharMobileTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ShikharMobileTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ShikharMobileTestRunner.getenv("SHIKHAR_MOBILE_TEST_LIVE")
        override = ShikharMobileTestRunner.getenv("SHIKHAR_MOBILE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ShikharMobileTestRunner.getenv(key)
        ...
        explain = ShikharMobileTestRunner.getenv("SHIKHAR_MOBILE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ShikharMobileTestRunner._test_control is not None:
            return ShikharMobileTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ShikharMobileTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ShikharMobileTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ShikharMobileTestRunner.load_test_control()
...
def load_env_local():
    ShikharMobileTestRunner.load_env_local()


def env_override(m):
    return ShikharMobileTestRunner.env_override(m)


def entity_data(v):
    return ShikharMobileTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ShikharMobileTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ShikharMobileTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ShikharMobileTestRunner.load_test_control()


def live_delay_ms():
    return ShikharMobileTestRunner.live_delay_ms()
