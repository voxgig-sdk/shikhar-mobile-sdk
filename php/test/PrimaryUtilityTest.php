...
class PrimaryUtilityTest extends TestCase
{
    ...
    private static function make_ctx_from_map(
        ?array $ctxmap,
        ShikharMobileSDK $client,
        ShikharMobileUtility $utility
    ): ShikharMobileContext {
        if ($ctxmap === null) {
            $ctxmap = [];
        }

        $ctx = new ShikharMobileContext($ctxmap, null);
    ...
    private static function fixctx(ShikharMobileContext $ctx, ShikharMobileSDK $client): void
    {
    ...
    private static function err_from_map(?array $m): ?ShikharMobileError
    {
        if ($m === null) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new ShikharMobileError($code, $msg);
    }
    ...
    public function test_make_error_no_throw(): void
    {
        ...
        $ctx->result = new ShikharMobileResult([
            'ok' => false,
            'resdata' => ['id' => 'safe01'],
        ]);
    ...
    public function test_feature_add_basic(): void
    {
        ...
        $feature = new ShikharMobileBaseFeature();
    ...
    {
        $hook_client = ShikharMobileSDK::test(null, null);
        $hook_utility = $hook_client->get_utility();
        $ctx = self::make_test_ctx($hook_client, $hook_utility);

        $called = false;
        $hook_feature = new class extends ShikharMobileBaseFeature {
            public $hook_fn = null;
            public function TestHook(ShikharMobileContext $ctx): void
            {
    ...
    {
        $init_client = ShikharMobileSDK::test(null, null);
        ...
        $init_called = false;
        $feature = new class extends ShikharMobileBaseFeature {
            public $init_fn = null;
    ...
    {
        $calls = [];
        $live_client = new ShikharMobileSDK([
            'system' => [
    ...
    {
        $blocked_client = new ShikharMobileSDK([
            'system' => [
    ...
    public function test_make_fetch_def_basic(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ShikharMobileSpec([
    ...
    public function test_make_result_no_result(): void
    {
        ...
        $utility = $client->get_utility();
        $ctx = self::make_test_full_ctx($client, $utility);
        $ctx->spec = new ShikharMobileSpec(['step' => 'start']);
        $ctx->result = null;
    ...
    public function test_make_spec_basic(): void
    {
        ...
        $spec_client = ShikharMobileSDK::test(null, $setup_opts);
    ...
    public function test_operator_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'operator', 'basic'), function (array $entry) {
            $in = $entry['in'] ?? [];
            $op = new ShikharMobileOperation($in);
            return [[
                'entity' => $op->entity,
                'name' => $op->name,
                'input' => $op->input,
    ...
    public function test_prepare_auth_basic(): void
    {
        ...
        $auth_client = ShikharMobileSDK::test(null, $setup_opts);
    ...
    public function test_result_basic_basic(): void
    {
        ...
        $this->runset(self::get_spec($primary, 'resultBasic', 'basic'), function (array $entry) use ($client, $utility) {
            ...
            if ($result->err !== null) {
                $err_msg = ($result->err instanceof ShikharMobileError) ? $result->err->msg : (string)$result->err;
