...
// ShikharMobile SDK feature test
//
...
class FtCtrl extends ShikharMobileControl
{
...
class FtClient
{
    ...
    private ShikharMobileUtility $utility;

    public function __construct(ShikharMobileUtility $utility, array $options)
    {
    ...
    public function get_utility(): ShikharMobileUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ShikharMobileContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ShikharMobileContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ShikharMobileContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ShikharMobileConfig::make_feature($name);
    ...
    private static function buildUrl(ShikharMobileSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ShikharMobileError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ShikharMobileResponse($fetched);
        }
    ...
    private function fail(ShikharMobileContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ShikharMobileContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
