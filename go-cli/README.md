...
boru-driven command-line client **and** interactive REPL for the ShikharMobile
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/shikhar-mobile-cli)
make build

# 2. See usage (words, entities, env vars)
./shikhar-mobile-cli --help
...
SHIKHAR_MOBILE_BASE=https://api.example.com ./shikhar-mobile-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/shikhar-mobile-cli
   ```
...
Run with no arguments to open a REPL (prompt `shikhar-mobile>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/shikhar-mobile-cli`. |
...
(`./shikhar-mobile-cli <expr>`) and interactively (the REPL), and expressions compose the
