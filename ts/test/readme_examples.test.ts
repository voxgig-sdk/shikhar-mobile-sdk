...
//     (\`new ShikharMobileSDK(...)\` / \`ShikharMobileSDK.test(...)\` become
//     \`ShikharMobileSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'ShikharMobileSDK'
...
// rewrite real client construction (\`new ShikharMobileSDK(...)\` and any
// \`ShikharMobileSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+ShikharMobileSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/ShikharMobileSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
