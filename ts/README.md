# ShikharMobile TypeScript SDK

The TypeScript SDK for the ShikharMobile API. Provides a type-safe, entity-oriented interface with full async/await support.
...
```ts
const client = new ShikharMobileSDK({ apikey: '...' })
...
| `tester(testopts?, sdkopts?)` | `ShikharMobileSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `ShikharMobileSDK.test(testopts?, sdkopts?)` | `ShikharMobileSDK` | Create a test-mode client. |
...
| `client` | `client(): ShikharMobileSDK` | Return the parent SDK client. |
...
shikhar-mobile/
├── src/
│   ├── ShikharMobileSDK.ts        # Main SDK class
