...
describe("AuthenticationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set SHIKHARMOBILE_TEST_AUTHENTICATION_ENTID JSON to run live")
      return
    end
...
function authentication_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("SHIKHARMOBILE_TEST_AUTHENTICATION_ENTID")
  ...
  if env["SHIKHARMOBILE_TEST_LIVE"] == "TRUE" then
  ...
  local live = env["SHIKHARMOBILE_TEST_LIVE"] == "TRUE"
  return {
    client = client,
    data = entity_data,
